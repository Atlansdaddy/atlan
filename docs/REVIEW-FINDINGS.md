...
| Preview proxy (:4590) unauthenticated loopback | all | **Mitigated 2026-07-24 (`7caee01`), row was stale until 2026-08-02.** `previewOriginOk()` (`preview.js`) rejects any `Host` this machine does not answer to (DNS-rebinding) and any cross-site `Origin` (cross-site fetch/WS) — the browser-reachable vectors are shut. **Gate widened 2026-08-05, and the doc said loopback-only until 2026-08-06:** the accepted set is no longer the loopback triple, it is `auth.js allowedHosts()` — loopback, `ATLAN_ORIGIN`, and the tailnet name derived from `tailscale status` at boot. Hardcoding loopback made the preview pane *structurally impossible on a phone*, and the shim written to work around that stripped the `Origin` header, re-opening the very vector this row claims is shut (measured: 403 direct to :4590, 200 through the shim). `Origin` is now matched by hostname rather than exact string, because the port a browser sees is a `tailscale serve` detail this process cannot know; suffix and prefix lookalikes are tested and rejected. **Residual, and it is the mobile-primary one:** a NATIVE local app can forge `Host` and omit `Origin`; on Android any installed app with INTERNET permission reaches loopback. Only a URL secret closes that, deliberately avoided. Same residual class as the `/api/auth/setup` row below. |
