...
| 10 | **Where does the human intervene?** | PLANNED | Policy decided 2026-07-28: an **autonomy ladder with regression** (`BUILD-SHEET.md` §8). Autonomy is granted **per stage** on *demonstrated* consistency, is **revocable**, and revocation **resets to the last stage that held** — a ratchet with a reverse gear, which a one-way "disengagement schedule" could not express. Human gates are held by John plus his son (volunteer) across stages 1–5, shrinking each stage until stage 7 needs none. Component still unbuilt, and the *measurement* of intervention count remains open (`BUILD-SHEET.md` #10). Reframing that made it tractable: **a human gate is a wall that has not been written yet** — attempt 2 did not lack a human, it lacked a render gate. |
...
| 16 | **Uneven walls across tiers** | **CLOSED** | Decided 2026-07-28. **Tier determines which operation an agent is assigned and which tools it gets — never the threshold.** The novice gaps the rings, the senior runs the dyno; the ring-gap spec does not change with who holds the gauge. Thresholds are **per-component, derived top-down from the application above** (`BUILD-SHEET.md` §0), and **blind to the builder** — otherwise `METHOD.md` §4's Haiku→Fable factor is uninterpretable, because target and instrument would move together, violating `METHOD.md` §2. See `BUILD-SHEET.md` §8 for the wall / containment / circuit-protection split, since "gated identically" is ambiguous across all three. |
