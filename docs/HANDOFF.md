...
- Known accepted gaps *(status re-verified 2026-08-02)*: **preview proxy (4590)** is Host+Origin gated since `7caee01` — the browser-reachable vectors are shut, but **a native app on the same device can still forge those headers**, which on Android is the realistic one; **builder/verifier Bash** is unscoped inside the project on proot (documented in-app) — `ATLAN_SANDBOX=1` confines it on a host with user namespaces, and the exec-mode CLIs are still ungated; brains providers see your prompts (their ToS applies).

## For a future AI picking this up

Read `docs/ARCHITECTURE.md` (the spine and the surface contract), then `docs/SPEC.md`, then `README.md`, then this file. Run `npm test` — **219 tests across 11 suites**, all green before and after your change. The durability rule: anything Termux/proot-fragile goes behind the adapter layer + a Doctor check. The product rule: honest framing always — budgets halt, scouts provably can't write, brains have no hands, missed routines never auto-fire. Never weaken those to "improve UX".
