...
| Function | Every HTTP endpoint contract + shape, plus data-store durability (corrupt/truncated JSON fails soft). (Spawns 1 tiny killed run.) | ✅ 41/41 |
...
WEB LIB SUITE
  ...
  ✓ a device code survives the ANSI colour around it
  ✓ a code boxed in a banner is not glued to the border
  ✓ a URL keeps its query string but drops trailing punctuation
  ✓ the LAST url wins, because a retry reprints the live one
  ...
  ✓ the class is named for the ROLE, not for a vendor
  ...
  ✓ an engine with no resume command gets NO hint offered
  ✓ the hint is whatever the ENGINE sent, verbatim — never assembled here
  ✓ a turn with no reported cost still renders a line
  ✓ an unknown profile means OFF, never "armed with something"
  ✓ only the three real profiles arm the gate
  ✓ the armed label says refusals are refusals, not more prompts
  ✓ auto-approve is remembered per CONVERSATION, not globally
...
FUNCTION SUITE
  ...
  ✓ key list marks which providers are testable; a bogus env is not
  ✓ POST /api/keys/test: unknown provider → ok:null; a set-less key → ok:false, never a fetch
...
CONNECTION SUITE
  ...
  ✓ a host with no tmux still gets a terminal, not an execvp error
...
ESLint as a gate rather than a suggestion. Exists because the linter's first run found a bug 846 tests never reached: agentExec.js re-exported killTree with `export { x } from`, which creates no LOCAL binding, so the run-timeout handler threw ReferenceError instead of killing the process tree — child left alive, uncaught exception in a timer callback. no-undef catches that whole class, a name referenced and never bound, and only keeps catching it if a violation fails the build. Runs the whole ruleset, not a chosen subset, because picking the important rules is how a config drifts from what CI enforces.

```
$ node test/lint.mjs
Lint
  ok  eslint reports no problems
  ok  eslint.config.mjs loads
...
Endurance report
  ...
  ok  a local night reports LOCAL MODEL WORK and claims no agent work
  ok  a model that went silent is counted, not averaged away
  ok  --local --dry-run states zero cost and refuses the fleet numbers
  ok  a run that stopped early says so, and does NOT say it survived
  ok  a run that reaches its window still passes normally
...
The third agent runner, and the first whose model runs on the user's own device — a 1B on a phone, given four tools. The interesting question is not whether it works but what happens when the model is WRONG, because GBNF constrains the SHAPE of a tool call and never its truth. Most of the file is a model asking for things it must not get. Six of these came from an independent adversarial review and each was REPRODUCED first: a symlink inside the project read another project's file and wrote outside cwd (resolve() is lexical, so it read as "under cwd", and guardPath re-checked the real path only against PROJECTS_DIR — two guards, neither watching this boundary); a 5MB file allocated 5MB of heap to return 60KB because the cap was on the string, not the read; a FIFO would have hung the request forever; and a stalled model server had no deadline at all.

```
$ node test/localagent.mjs
LOCAL AGENT
  ✓ an API provider is driven with a Bearer header + prior history (chat model = coding agent)
  ✓ the keyless local path sends NO auth header
  ...
  ✓ a path ESCAPE is refused, and the model is told why
  ✓ an absolute path outside the project is refused
  ✓ the SCOUT profile cannot write — the same predicate the fleet uses
  ✓ an unknown tool is refused rather than guessed at
  ✓ an unknown PROFILE refuses instead of defaulting to a permissive one
  ✓ a model that only ever calls tools is stopped by the STEP cap
  ✓ the TOOL-CALL cap catches a model batching calls inside one turn
  ✓ unparseable tool arguments become a recoverable error, not a crash
  ✓ a server started without --jinja is named exactly, not reported as a 500
  ✓ an oversized write is refused before it touches the disk
  ✓ a truncated read SAYS it was truncated, in what the MODEL receives
  ✓ an empty prompt and a missing cwd are refused
  ✓ a SYMLINK inside the project cannot read outside it
  ✓ a DEEP path under a symlink cannot outrun the resolver
  ✓ a SYMLINK inside the project cannot write outside it
  ✓ a huge file is refused on its SIZE, not read and then trimmed
  ✓ a FIFO is refused instead of blocking the request forever
  ✓ list_files refuses a file, and marks dirs from ONE readdir
  ✓ every model request carries a deadline, so a stall cannot hang forever
  ✓ a caller-supplied signal is honoured over the default deadline
  ✓ a declined tool does not run, and the model is told plainly
  ✓ the gate is asked BEFORE the tool runs, not after
...
What a conversation keeps when a turn does NOT go well. Every streaming engine buffers its output in memory until the turn ends, and "ends" used to mean chat.result alone — so a turn that streamed a real answer and then failed wrote nothing at all. The answer was on screen, which is precisely why it looked saved, and it was gone on reopen. The local model's intermittent 500 makes that the everyday case rather than an edge one. Driven frame by frame with an injected sink, so it needs no engine, no network and no llama-server.

```
$ node test/transcript.mjs
transcript recorder
  ✓ a streamed turn that SUCCEEDS is written once, complete
  ✓ a streamed turn that FAILS mid-answer still keeps what it streamed
  ✓ err then result cannot write the same partial answer twice
  ✓ a failure with nothing streamed writes nothing
  ✓ whitespace-only stream is not a turn worth keeping
  ✓ a failed turn does not leak its text into the next one
  ✓ whole-message engines are recorded as they arrive
  ✓ no conversation id means nothing is written
  ✓ a malformed frame is ignored, not fatal
  ✓ two connections keep separate buffers
