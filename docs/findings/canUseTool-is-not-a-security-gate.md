...
**Status: STILL VALID — re-verified 2026-08-04 against `@anthropic-ai/claude-agent-sdk` 0.3.221 (bumped from 0.3.210 per rule 3, black-box test re-run on the new pin: scout ordered to write a canary file, file never appeared).** The mitigation is present in Atlan at `fleet.js:178,183` (`settingSources: []` + per-profile `disallowedTools`) and `claudeEngine.js:88`, where the comment correctly labels `settingSources:[]` a security boundary rather than a preference. The re-check is now automated: `test/e2e.mjs` carries the canary test, so every future bump re-proves this finding instead of trusting it.
