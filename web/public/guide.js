...
(() => {
  ...
  const STEPS = [
    { s: 's-chat', el: 'header .atlan', h: 'This is Atlan', p: 'The steady presence in your cockpit — and he\'s ALIVE to what\'s really happening: teal glow = calm, shimmering = agents or a build running, orange pulse = something needs you, bright flash = something just shipped. The little lights orbiting him are your fleet agents, one per running agent. The line under his name is him talking to you.' },
    ...
    { s: 's-chat', el: '#projSel', h: 'Pick your project', p: 'Every repo in your projects directory with git or package.json shows up here. Chat, fleet runs, builds and the terminal all work inside whichever project you pick here.' },
    { s: 's-chat', el: '#modelSel', h: 'The engine switcher', p: 'Five honest groups. "Escalate" is the LADDER: it asks the free on-phone model first and climbs to a paid rung only when it must — usually the right default on a phone. It climbs on things code can SEE (the rung errored, came back empty, got cut off, or said "I can\'t") and shows you every step. It deliberately does NOT ask a model to grade its own answer, so it cannot catch a confidently WRONG one — if a reply looks off, switch to a stronger engine. That limit is written down, not hidden. "Claude Code" = the real agent: files, tools, builds — it can DO things. "Agent CLIs" = Codex, Antigravity, Grok Build and Copilot — full agents in your repo, each with its own permission system. Atlan runs them with approvals bypassed today; per-engine levels (full auto / gated / plan-only) are coming. "On-phone" = free local models, chat only. "Cloud brains" = smart but handless: they talk, they can\'t touch. Greyed options tell you exactly what they need to wake up.' },
    ...
    { s: 's-chat', el: '#chatlog', h: 'The session is portable', p: 'After each turn a grey line shows the cost and the session id. Tap it to copy "claude --resume <id>" — paste that in the Term tab (or any terminal on the host) and the SAME conversation continues on the command line. GUI and CLI are one brain here.' },
    ...
    { s: 's-editor', el: '#editor', h: 'Editor — write or review code by hand', p: 'A full code editor (122 languages, syntax-highlighted). Type a file path and ↵ to open, or ☰ to browse the project tree. Edit, Save straight to disk. "Send to Claude for review" attaches the file to a chat and asks for feedback — so you and the agent trade off on the same code. Scoped to your project; secrets like .ssh are off-limits.' },
    { s: 's-term', el: '#term', h: 'A real terminal', p: 'A real shell in your project. On Linux hosts it\'s tmux session "atlan-main" — type "tmux attach -t atlan-main" in any other terminal and you take over THIS exact screen, live, both ways. The GUI never locks you in.' },
    ...
    { s: 's-fleet', fp: 'fp-runs', el: '#fleetBudget', h: 'Budgets are a WALL', p: 'The budget is not a warning — at the cap the run halts. It\'s checked between steps, so one in-flight step can overshoot the cap a little before the halt lands, but there\'s no unbounded spend and the ledger always shows the true number. No $10k weekends. Note: an agent\'s first turn alone costs ~35k tokens (system prompt), so 50k is the practical floor.' },
    ...
    { s: 's-doctor', el: '#keysList', h: 'LLM API keys', p: 'API keys live here, encrypted (AES-256-GCM) on disk, shown back as last-4 only, never echoed anywhere — split into LLM and Voice groups. Environment variables always win over stored keys.' },
    { s: 's-doctor', el: '#doctorList', h: 'Doctor — the fragile bits, watched', p: 'Every fragile host assumption that could break with an update has a check: JDK, SDK, aapt2 shim, claude binary, auth, tmux, disk, the push service worker\'s no-cache promise. Green = go. Red = this screen tells you exactly what broke.' },
    { s: 's-doctor', el: '#preflightList', h: 'Preflight — the exposure gate', p: 'A different question than Doctor: not "does it work" but "is it safe to show beyond this phone". Loopback binding, a password set, encrypted keys, no plaintext files, gitignore coverage, no live tunnels. These should ALL be green before Atlan is ever tunneled anywhere — and even when they are, the app stays loopback-only until you deliberately expose it.' },
    { s: 's-doctor', el: '.lore', h: 'That\'s the tour', p: 'That\'s Atlan — a calm place to build, that lights up when your work is moving and keeps watch while your agents run. Everything you make here surfaces eventually. Tap ? anytime to reread any of this. Now go build.' },
  ];
